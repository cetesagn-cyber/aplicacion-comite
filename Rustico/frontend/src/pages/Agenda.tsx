...
interface Cita {
  ...
  cliente_nombre: string; servicio_nombre: string;
...
function slotDeCita(inicio: string): number {
  ...
  const horas = d.toLocaleTimeString('es-CO', { timeZone: 'America/Bogota', hour12: false });
  const [h, m] = horas.split(':').map(Number);
  return Math.round(((h * 60 + m) - HORA_INICIO * 60) / SLOT_MIN);
}

function duracionEnSlots(inicio: string, fin: string): number {
  return Math.max(1, Math.round((new Date(fin).getTime() - new Date(inicio).getTime()) / (SLOT_MIN * 60_000)));
}

function formatHora(iso: string) {
  return new Date(iso).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Bogota', hour12: false });
}
...
export default function Agenda() {
  ...
  const [form, setForm]           = useState({ barbero_id: '', servicio_id: '', cliente_id: '', inicio: '', notas: '' });
  ...
  const abrirModal = async () => {
    ...
    setForm({ barbero_id: barberos[0]?.id || '', servicio_id: servicios[0]?.id || '', cliente_id: '', inicio: '', notas: '' });
    setErrorForm('');
    setModal(true);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      {/* Barra superior */}
      <div style={{
        padding: '16px 24px', borderBottom: '1px solid var(--borde)',
        background: 'var(--superficie)',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => setFecha(d => addDias(d, -1))} style={{ background: 'none', border: '1px solid var(--borde)', borderRadius: 6, padding: '6px 8px', color: 'var(--texto-suave)' }}>
            <ChevronLeft size={16} />
          </button>
          <span style={{ fontSize: 16, fontWeight: 600, color: 'var(--blanco)', textTransform: 'capitalize', minWidth: 240, textAlign: 'center' }}>
            {fechaLabel}
          </span>
          <button onClick={() => setFecha(d => addDias(d, 1))} style={{ background: 'none', border: '1px solid var(--borde)', borderRadius: 6, padding: '6px 8px', color: 'var(--texto-suave)' }}>
            <ChevronRight size={16} />
          </button>
          <button onClick={() => setFecha(new Date())} style={{ background: 'var(--superficie2)', border: '1px solid var(--borde)', borderRadius: 6, padding: '6px 14px', color: 'var(--texto-suave)', fontSize: 13 }}>
            Hoy
          </button>
        </div>
        <button
          ...
          style={{
            ...
            padding: '8px 16px', color: 'var(--blanco)', fontWeight: 600, fontSize: 14,
          }}
        >
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={crearCita} disabled={guardando} style={{ flex: 2, padding: '10px', borderRadius: 8, border: 'none', background: 'var(--verde)', color: 'var(--blanco)', fontSize: 14, fontWeight: 600 }}>
